...
//  var getData = function(numSeries, numRows, isStacked) {
  function getGraphData (numSeries, numRows, isStacked) {
    var data = [];
...
//  var makeGraph = function(className, numSeries, numRows, isStacked) {
  function makeGraph (className, numSeries, numRows, isStacked) {
    var demo = document.getElementById('graphCanvas');
    ...
    var g = new Dygraph(
      div,
      getGraphData(numSeries, numRows, isStacked),
      {
        width: 480,
        height: 320,
        labels: labels.slice(),
